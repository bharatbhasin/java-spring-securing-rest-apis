...
public class ResolutionAuthorizer {
	...
	public boolean authorize(MethodSecurityExpressionOperations operations) {
		...
		String name = operations.getAuthentication().getName();
		Optional<Resolution> resolutions = (Optional<Resolution>) operations.getReturnObject();
		return resolutions.map(Resolution::getOwner).filter(owner -> owner.equals(name)).isPresent();
	}
}
